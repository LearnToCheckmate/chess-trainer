...
T=[
 ...
 ("lessons","Lesson walkthrough videos (Hanging Pawns, in-app)","part","#330","84 of 170 lessons have a Hanging Pawns walkthrough (#324 +21, #328 +11, #330 +2 endgames, the first endgame videos). Every ID read off the channel and confirmed on its watch page. Most remaining lessons have no HP video at all; the searched-and-empty list is in the backlog."),
 ...
 ("lessons","Lesson videos (embedded player + curated IDs)","done","#330","Duplicate of the Hanging Pawns walkthrough row; the player and the curated IDs are the same feature. Deduped 2026-09-08."),
 ...
 ("social","Player rating for human games (Elo, K decays with games played)","part","#349","Start 400, floor 100. Established equal win +10, new-player upset up to +51. Math fully unit-tested (21 assertions incl. convergence and drift). Computed on the phone, so it is editable by a determined user; the pure functions can move to a Cloud Function later. Live two-player exchange still needs Kunal and a second device to confirm."),
 ...
 ("polish","Home queen-drop animation restored on phones","done","#347","Only ever rendered on tablet portrait or landscape; HomeIntro already had a compact mode that was never used on phones."),
 ...
 ("review","Best move sequence written out, not just animated","done","#344","Tapping best now prints the whole line it leads to."),
 ("polish","Bottom safe-area inset measured","done","#344","Only the top inset was measured. The ~34px home-indicator area was being spent as if free, which is why gates passed at 430x932 and the screen still did not fit on a real phone."),
 ("review","Why a move got its verdict (coach reason line)","done","#343","Composed from the loss, the opponent's best reply (the punishment), the move that held it, and the tactical motif. Example: \"A blunder, about 4.1 pawns of advantage gone. Ng4 is the punishment. Nxd6 held it.\""),
 ("review","Analysis board from any reviewed position","done","#343","Analyze hands you the board at that ply. Both colours movable, Undo, Exit, and the engine line follows what you play."),
 ("review","Review speed: parallel Stockfish workers","done","#343","61s -> 27s measured on a 44-move game, AND more thinking time per position than before. Pool sized off the device, torn down after the review. localStorage ct_pool overrides it."),
 ("review","Multi-threaded Stockfish (COOP/COEP + threaded build)","open","","NOT done and deliberately deferred, see #343. COEP require-corp would cut off Firebase/gstatic, the chess.com API and Lichess; credentialless is the only variant worth trying and its Safari support is the open question. The threaded binary is also not obtainable from the Cowork sandbox. Parallel workers were built instead and beat threads for this workload."),
 ("polish","Base shell for every board screen","part","#345","Review DONE (#344). Play DONE (#345): board 416 -> 424, bars absorb the slack, black band 237px -> 0. Puzzles DONE by side effect (424/384). LESSONS still open, and it is a content decision: 61px coach line + 205px text block + 99px moves panel against \"one context line\". Needs Kunal\u2019s word before deleting lesson prose."),
