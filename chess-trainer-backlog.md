...
## Phase E scouting (2026-06-12, pre-build)
- Solve trigger: setPuzSolved(true) at the sol.length branch (~line 2508); done-record + recordSolve + existing PzCelebrate rank-up celebration + 🎉 puzMsg at ~2511. Per-solve banner judged too noisy; Phase E centerpiece = the path visuals (gradient nodes, rings, progress draw) on the puzzle map, next run.
