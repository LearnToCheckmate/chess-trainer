# ACTIVE QUEUE - reconciled 2026-09-12 (app at build #349 LIVE; Cowork session, single writer)

## 2026-09-12 - PROCESS - Feedback inbox, and a reading failure worth recording
- KUNAL: "whenever you're finishing up one of these instructions before you fully finish check the feedback chat... keep looking at the feedback chat, pick up the feedback, continue, and only when there's no more feedback for you to pick up, that's when you give me the results."
- THE LIMIT, stated to him rather than papered over: a SIBLING CLAUDE CHAT CANNOT BE READ FROM A COWORK SESSION. Project docs, the repo and artifact databases are reachable; another conversation is not. Writing "check the feedback chat" into the protocol would have been writing down a step that silently does nothing, and he would have got silence instead of an error. Built a Feedback Inbox artifact instead (7b876dc4-be89-4f73-a839-3a454cab40ef, collection "items", fields text/at/status/build) and put THAT in the protocol: he types items from his phone whenever they occur to him, Claude drains it before every close-out and stamps each handled item with the build that answered it.
- THE PROTOCOL, now in HANDOFF section 4 and in project memory: finish the work, READ THE INBOX, build what is new, read it AGAIN, and only write the close-out when a read comes back empty. Never close out on a single read.
- A READING FAILURE I OWN. The Review Screen Audit artifact was read ONCE early in this session and then reported as "untapped" in FOUR consecutive close-outs. Kunal had tapped all seven items on 2026-09-11 at 17:32. I cached a stale "nothing there" and repeated it. The rule added to HANDOFF: re-read every feedback db on every run, never carry an empty answer forward.
  - HIS ACTUAL VERDICTS, reconciled against what shipped: R1 approve (one-screen default - shipped #339). R3 keep (move strip - present). R4 keep in the row (key-moment button - present, now the star icon). R5 keep the chip (best-move chip on the move line - present). R2 discuss (eval number along the bar vs a wider bar) - OVERTAKEN by his own later instruction to run the bar horizontally above the board, which is the better version of option B. R6 discuss (top of screen, and whether the players return somewhere small) - ANSWERED since: the players are now the two bars around the board. Q1 icon: he opens from the HOME SCREEN, which is exactly the fact that made the unmeasured bottom safe-area inset matter in #344, and I found that independently three builds later instead of reading it here.
  - NOTHING he tapped contradicts what shipped. The cost of the miss was time, not a wrong build.
- CONFIRMED BY KUNAL: the review is faster since #347. That closes the one gap I could not measure from the sandbox.


## 2026-09-12 - BUILD #349 - A real rating, built by playing people
- KUNAL: "as we enable this where users are playing each other they should be able to build a rating like they do on chess.com... everybody starts from 400... for every game they win against somebody that's almost closely ranked they get about eight points... if you're playing with somebody significantly stronger than you then sometimes you can even get up to 50 points."
- THE ARITHMETIC PROBLEM HE HAD ALREADY SPOTTED WITHOUT NAMING IT: a single Elo K-factor cannot produce both "about 10 for a close game" and "up to 50 for an upset". If an equal win is worth 8, beating someone 400 points stronger caps near 15. chess.com gets both because Glicko's rating deviation makes new players move fast and settled players move slowly. Same effect here with a K that decays on games played: <10 games K=56, 10-29 K=32, 30+ K=20, and 30+ above 2100 K=12. Start 400, floor 100.
  - MEASURED, established (30+ games, 1000): equal +10/-10, +100 opponent +13/-7, +400 opponent +18/-2, -200 opponent +5/-15. New player (<10 games, 400): equal +28, +200 upset +43, +400 upset +51. Exactly his spec at both ends.
- A SIMULATION CAUGHT A REAL BUG I HAD WRITTEN. The first version forced every result to move at least one point, so that "something always happens". That breaks Elo's fixed point: a player in a stronger pool gains ~+20 on a rare win but pays a forced -1 on every expected loss. MEASURED drift: -0.581 points per game for a 400 in a 600-1800 pool, -0.122 for a 1000 in the same pool. Over a few thousand games it sinks a correctly-rated player to the floor. Removed, and the rating is now kept as a FLOAT and rounded only for display so repeated rounding cannot leak either. Same measurement after: 0.002 and -0.007 points per game.
  - AND THE FIRST TEST I WROTE FOR IT WAS ALSO WRONG, which is worth recording. It generated each result from the player's OWN CURRENT rating, which makes the walk driftless by construction and proves nothing; it just wanders until the floor absorbs it. The meaningful test gives the player a TRUE strength, generates results from that, and asserts the rating finds it: true 1200 lands at 1271, true 750 lands at 798, both inside 180 points after 3000 games.
- HOW THE EXCHANGE WORKS WITHOUT A SERVER REFEREE: each player's rating is written into the game record at create/join (index.html `_gameCreate`/`_gameJoin` now take a meta argument), so when the game ends BOTH devices run the same pure function over the SAME two starting ratings and agree. Guarded by a list of already-rated game ids in localStorage, because the watcher reports a result more than once. An opponent on an older build has no rating in the record and no exchange happens at all, rather than one side inventing a number.
- HONEST LIMIT, and Kunal was told before it was built: this is computed on the phone, so a determined person could edit their own stored rating. `eloDelta` and `eloK` are deliberately PURE and dependency-free so the identical code can move into a Cloud Function later and produce identical numbers. That needs a functions deploy, which cannot be done from this sandbox.
- NOT VERIFIABLE HERE: two real players cannot be put against each other from the sandbox. The MATH is fully covered by elo349.js (21 assertions: the exact quoted table, symmetry, draws, floor behaviour, convergence on a true strength, and the drift measurement). The live exchange needs Kunal and a second device.
- HUMAN GAMES ONLY, on purpose. A rating earned against a bot whose strength you chose yourself would be meaningless, so vs-computer games do not touch it.
- GATES: compile clean, and index.html's classic script blocks parsed as a separate check since the cloud layer changed. Review gate 52 assertions PASS. Shift check PASS. Play/lesson regression PASS. elo349.js PASS. Brilliant-gate guard unchanged.


## 2026-09-12 - BUILD #348 - Player ratings and country flags
- KUNAL: "whatever information we have about the players, either in the review or when we are playing, we should display that. So the ratings from the players, if they're from a particular country, the flags like chess.com does."
- RATINGS WERE FREE, and that is the interesting part. `parsePGNHeaders` already captures EVERY header with a generic regex, and chess.com and lichess both write `WhiteElo` / `BlackElo` into the PGN. We had the numbers in `review.headers` the whole time and were ignoring exactly those two keys. They now render beside each name in both bars, validated as 3-4 digits so a malformed header shows nothing rather than garbage. In Play the COMPUTER's bar shows the Elo of the bot you chose.
- WHAT I DELIBERATELY DID NOT DO: put a rating on Kunal's own bar in Play. `cpuElo` is the strength he is playing AGAINST, nudged by results. Showing it as his rating would be inventing a number, so his side stays blank there. Same rule everywhere: no data, no pill.
- FLAGS NEED A LOOKUP, because no PGN carries a country. `fetchCountry(site,user)` hits api.chess.com/pub/player/{user} (`country` is a URL whose last segment is the ISO code) or lichess.org/api/user/{user} (`profile.flag`), and `flagOf(cc)` turns the code into an emoji via regional indicator letters. Cached in localStorage under `ct_country` FOREVER, misses included, so a player with no country set is never looked up a second time. The effect fires once per reviewed game, skips the network entirely when both players are already cached, and any failure just means no flag.
- NOT VERIFIABLE IN THIS SANDBOX, and it must be said plainly: api.chess.com is blocked here. The gate confirmed the fetch FIRES (the proxy logged six blocked CONNECTs to api.chess.com) and that a blocked lookup breaks nothing, but the flags themselves can only be confirmed on Kunal's phone. The rating half IS verified: the gate asserts 1187 and 1243 out of the demo PGN at both viewport sizes.
- The demo PGN in all eight gallery cards now carries `WhiteElo`, `BlackElo` and `Site`, so what a card shows matches what a real chess.com game shows.
- GATES: compile clean. Review gate PASS including the new rating assertion. Shift check PASS: the bars still never change height with the extra pills, which was the risk. Play/lesson regression and puzzle gate re-run.
- KNOWN INTERMITTENT, recorded rather than hidden: one shift run reported `pageerror RuntimeError: unreachable`, a Stockfish wasm trap, and did not reproduce on an immediate re-run or in any other gate. Not caused by this build as far as I can tell, but if it recurs it is the engine worker, most likely around teardown. Watch for it.


## 2026-09-12 - BUILD #347 - Eight items from Kunal's screenshot batch
- 1. BOARD STILL JUMPED as captures accumulated ("after move 12 when you would move 13 the size of the box with the name of the person on top it expands and that makes the whole chessboard jump up and down"). The captured-pieces row inside the player bar was `flexWrap:'wrap'` with `minHeight:18`, so once enough pieces were taken it WRAPPED to a second line, grew the bar, and moved the board. Now one clipped line at a fixed 18px. The shift harness gained a late-game pass that steps 16 plies with most captures on the board and asserts the bars never change height: before, they did; now "116/116" is the only value seen.
- 2. REVIEW OVER A MINUTE, and BOTH causes were mine.
  - (a) THE CACHE TAG. #341 keyed the eval cache on `sans|movetime|sf18`; #343 changed it to `sans|sf18c`. That silently invalidated every review Kunal had ever cached, so a game he had already reviewed ran from scratch again. He noticed exactly this: "I had done a review in the previous version, it was much quicker." The tag now carries a comment telling the next session not to bump it without a reason.
  - (b) THE BUDGET. #343 spent the parallel saving on DEPTH, taking per-position thinking time from ~1333ms to ~2270ms, on the strength of a 27s measurement in this sandbox. On a phone with fewer workers and slower cores that wipes out the speedup entirely and can land slower than the old sequential build. The budget is now a WALL-CLOCK TARGET (24s): it starts deliberately low, and after one full round of the pool it measures real elapsed time, projects the total, and scales the per-position budget to land on target. It also says so on the progress screen when it trims.
- 3. BOARD SMALLER THAN THE SCREEN. This was the reserved rows from #346 plus #345's second control row. MEASURED at his real usable height (430x839, i.e. 932 minus the 59px status bar and 34px home indicator): board 424px, 99% of screen width, which is the maximum the screen allows. At 428x833 it is 416px, 97%. Full breakdown recorded: header 40, topBar 72, evalStrip 22, moveLine 38, why 38, controls 48, strip 52, bottomBar 72 = 382px of chrome. The bars are slack absorbers, not cost: the board is already at its WIDTH cap, so there is nothing further to reclaim on a 430-wide phone.
- 4. ICONS TOO SMALL. The control buttons were still sized for the text labels they stopped carrying in #344. Icon-only buttons now render at clamp(21px,5.4vw,26px) with a 48px row, and every one carries an aria-label as well as a title.
- 5. THE TO-MOVE RING did not pop on the light bar. It was one colour at 2px that reads well on the dark bar and disappears on the cream one. Now 3px, and on the light bar it gets a second 2px dark ring outside it for contrast.
- 6. HOME ANIMATION. The queen-drops-in intro only ever rendered when `hbig` (tablet portrait) or `hLand` (landscape) was true; a phone in portrait always got a static title row. HomeIntro already had a `compact` mode built for this size and it was simply never used on phones. Restored, and tapping the title replays it.
- 7. "NEW TO CHESS? LOCK IN THE ENDGAME BASICS" DID NOTHING. One-word bug: the handler called `selectOpening(_coachRec.i)` but the object only has `.idx`, so it passed undefined and left you wherever you were, which read as "it just takes me to Play". The copy of the same handler in the Coach sheet had `.idx` correct all along.
- 8. A BRILLIANT MOVE DID NOT SAY WHY in the brilliant-move trainer: it replayed the position and said "That's your brilliant move. Nicely done." and nothing else. The reason is now composed when the brilliancy is captured (sacrifice size, the eval it held, the tactical motif) and stored with the position, so the card explains it. Records saved before this build have no reason and fall back to the plain line rather than showing an empty space.
- GATES: compile clean. Shift check PASS including the new late-game capture pass. Review gate 50 assertions PASS. Play and lesson regression PASS. Puzzle gate PASS. Chrome breakdown recorded at two real usable heights. Brilliant-gate guard unchanged.


## 2026-09-12 - BUILD #346 - The board holds still
- KUNAL: "Every time I play a move, it's a computer thinking on top making the whole screen the chessboard move very distracting. Please fix it and make sure it doesn't happen anywhere else either." FOUR causes, and two of them were mine from the last two builds.
- CAUSE 1, the one he could see. "Computer thinking..." was a CONDITIONALLY RENDERED row (minHeight 18 + marginBottom 7 = 25px) that was inserted when the engine started and removed when it stopped, on every single move. It now always occupies its 18px and only the text inside it changes, with opacity for the fade.
- CAUSE 2, MINE, from #344/#345. The fit loop listed `playHist.length` among its dependencies AND called `setBoardTrim(0)` on every run, so each move reset the board to full size and then shrank it back over the next frames. Split the deps: a `_geoKey` (viewport, safe areas, mode, screen identity, eval-bar mode) now decides whether to RESET, and content changes settle from the CURRENT trim instead. Added a slow grow-back for genuine sustained slack (>24px) so the board still recovers size when content really does shrink.
- CAUSE 3, found by going looking rather than waiting for him to report it. The player bars absorb slack by design, which means ANY content change below the board moves the board. The reason line under a move is one or two lines depending on the move, so stepping through plies moved the board 8px each time. The reason block now holds a constant height: two lines for the reason, one for the engine line when the readout is on, reserved whether or not there is text.
- CAUSE 4, same class. The strength selector (bot name, Elo pill, slider) hid itself on the FIRST MOVE, freeing ~111px, letting the bars grow and jumping the board 26px at exactly the moment Kunal played his first move. It is a duplicate of the control on the play SETUP screen, so it is now hidden for the whole live game: the layout settles before he touches a piece, and the board and bars get that space from the first paint. The play move-history row was reserved for the same reason.
- HARNESS: shift346.js plays a real move on the play screen and samples the board's top and height 26 times across the computer's think, then steps 14 plies in review and samples again. It asserts ONE distinct top and ONE distinct height per screen. Before: play tops [234, 244] with the board resizing mid-think; review tops [192, 200]. After: play [242] and review [190], single values, and the first move no longer moves the board at all. RUN THIS ON ANY CHANGE THAT ADDS OR REMOVES A ROW ON A BOARD SCREEN.
- THE GENERAL RULE, now in the HANDOFF: on a board screen, a row that can appear or disappear must reserve its space. Because the bars flex to absorb slack, a row appearing anywhere - above OR below the board - moves the board. Conditional rendering of a row is the bug; conditional rendering of its CONTENT is fine.
- COST, stated plainly: reserving the reason block costs board width on short screens. At 430x745 the board is 368 where it was 384. That is the price of a board that does not move, and it is the right trade.
- GATES: compile clean. Shift check PASS. Review gate 50 assertions at 430x932 and 390x844 PASS. Play and lesson regression PASS. Puzzle gate PASS. Fit sweep re-run. Brilliant-gate guard unchanged.


## 2026-09-12 - BUILD #345 - Verdict colours in the move strip, a reason line that says something on good moves, and the base shell on Play
- KUNAL, from a screenshot of a Brilliant: "if this move is brilliant, then at the bottom we should have that colour for brilliant. I noticed you have the colour for some of the other moves... figure it out across the board."
- HE WAS RIGHT AND IT WAS WIDER THAN THE CASE HE SPOTTED. The strip chip read `color: bad ? a.cls.c : neutral`, `background: isCur ? 'rgba(255,216,77,.28)' : 'transparent'`, `{p.san}{bad ? a.cls.i : ''}`, where `bad` was ONLY Inaccuracy, Mistake and Blunder. Three defects at once: (1) the selected-move chip was a hard-coded amber that overrode the verdict entirely, which is the gold-instead-of-cyan he saw; (2) only the NEGATIVE half of the scale had any colour, so Brilliant, Great and Miss - the three you most want to find while scrubbing - were the ones invisible; (3) their symbols were dropped too, so blunders showed ?? but Brilliant never showed !! and Great never showed !. Now `notable` covers Brilliant, Great, Miss, Inaccuracy, Mistake and Blunder; the chip takes the verdict colour at 3d alpha with a matching aa outline; symbols come through. Best and Good stay neutral deliberately, or the strip becomes a rainbow and the key moments stop standing out.
- REASON LINE ON GOOD MOVES. Also from his screenshot: a Best move read "The engine's first choice. Black is losing here." True and useless. The explainer was built for mistakes and restated the label for everything else. It now spends the runner-up gap it was ALREADY computing for the Great check: ">=1.0 pawns" becomes "Nothing else came close: <alt> was about N pawns worse", ">=0.35" becomes "the only other try", below that "<alt> was just as good". Good and Book branches got the same treatment. Compare the Brilliant line, which was always doing its job: "you gave up material and the position still reads +3.5 for white. It forks two pieces at once."
- BASE SHELL, SCREEN 2 OF 4 (PLAY). Board 416 -> 424 at 430 wide and 376 -> 384 at 390: the `reserved` calculation was still subtracting 4px for the VERTICAL eval bar even when the bar runs horizontally above the board, which costs 8px after the /8 rounding. Player bars now flex on play exactly as they do in review, and the board column can hand them the leftover height. MEASURED: the black band at the bottom of the play screen went from ~237px to 0px, at both 430x932 and 390x844.
  - PUZZLES got the wider board for free from the same `reserved` fix: pzgate now measures 424 and 384 where it measured 416 and 376, still with no page scroll and no empty band.
  - THE FIT LOOP IS NOW GENERAL, per the HANDOFF note telling the next session to reuse it rather than copy a constant per screen: `_fitScreen` covers the one-screen review, play, puzzle browse/online and an open lesson. `boardTrim` applies on all of them.
- RANK 1 IS NOT CLIPPED. I suspected from a screenshot that the last rank was being squeezed and said I would measure rather than guess. Measured at 430x932, 428x926, 390x844 and 375x548: all eight ranks identical to the pixel (53/52/48/24), the ranks sum exactly to the grid height, and the last rank clears the player bar by 2px everywhere. What looked like clipping is the file letters rendering inside rank 1 plus the 3px board border. No defect; recorded so nobody re-opens it.
- GATES: compile clean. Review gate 430x932 and 390x844, 50 assertions, GATE PASS. Puzzle gate GATE PASS with the new 424/384 widths. Play and lesson regression PASS, including two new assertions (player bars present on play, and no black band at the bottom). Brilliant-gate guard unchanged.
- STILL OPEN ON THE SHELL: LESSONS. The board is 424 there now and the fit loop keeps it on screen, but lessons still carry a 61px coach line, a 205px text block and a 99px moves panel, against Kunal's "one context line". That is a CONTENT decision, not a layout one - deleting lesson prose unilaterally would be the wrong call - so it needs his word first.


## 2026-09-12 - BUILD #344 - The review screen fits every phone, not just the ones I happened to test
- KUNAL: "this is a good layout. still doesn't fit the screen", then "different phones are gonna have different heights and different widths... we should do something that works across all phones, not just mine." Both correct, and the second one is why this build does not ship a tuned number.
- ROOT CAUSE, and it is worth stating plainly because I had been testing the wrong thing. On a phone the board was sized from screen WIDTH ONLY. There WAS a height cap, but (a) it was written before the player bars and the second control row existed so it under-counted the chrome by ~124px, and (b) it carried a `Math.max(296, ...)` floor that overrode the cap outright on any short screen. The board therefore never shrank and the move strip simply fell off the bottom. MEASURED before the fix: 430x745 cut off by 89px, 390x664 by 122px, 375x548 by 158px.
- SECOND ROOT CAUSE, mine: the app measured `env(safe-area-inset-top)` but never the BOTTOM inset. On a notched iPhone installed to the home screen that is ~34px of home-indicator area, and with the 59px status bar it means my 932px test viewport was handing the layout ~93px that Kunal's phone does not have. That is most of why every gate passed here and the screen still did not fit for him. safeBot is now measured the same way safeTop always was, and both come out of the height budget.
- THE FIX IS A MEASUREMENT, NOT A CONSTANT. The analytic cap (REV_CHROME) is only a first guess so the first paint is close. After paint, a useLayoutEffect reads the REAL `scrollHeight - clientHeight` and shrinks the board until the overflow is zero. Shrink-only within a settle cycle, with an 8px cushion; the trim resets to 0 whenever the screen or the content changes, which is what gives the board its size back. Growing back inside the same cycle is what made the first attempt oscillate and land 4px over. Bounded at 24 passes, and it never runs outside the one-screen review.
  - This is the part that answers Kunal's question properly: no constant can know about a wrapped player name, a two-line reason, a mate score, a larger accessibility font, or a phone that does not exist yet. A measurement can.
- MEASURED AFTER, 14 viewports, ALL FIT with zero overflow: 430x932 board 424, 430x839 (Pro Max minus notch and home bar) 424, 430x745 (Safari) 384, 393x852 384, 393x759 384, 390x844 384, 390x664 (Safari) 304, 412x915 Pixel 400, 412x732 368, 360x800 352, 360x640 280, 375x667 304, 375x548 192, 320x568 208.
- KUNAL, SAME SESSION: "Previous and next buttons are redundant because we already have the previous and next buttons at the bottom across the move list" and "next key moment and analyze... you can just make them icons so the buttons don't take so much space". Both done, and they are part of the fit fix rather than separate: dropping Prev/Next and shrinking the labels to icons collapsed TWO control rows into ONE and handed 50px straight back to the board. The row is now first / play-pause / last / star / analyze, each with a title; in analysis it becomes Undo and Exit analysis, which keep their words because they are modal.
- KUNAL: "my name's written in black, and I'm playing white... it's a little confusing". The side was encoded only in the bar's background colour, and the name's contrast colour reads as the opposite side. Each bar now says WHITE or BLACK in words beside the name. Colour is no longer the only carrier of that information, which is an accessibility win as well.
- KUNAL, EARLIER AND ONLY HALF DONE UNTIL NOW: "they have the ability to show the best move and a sequence as to what happens when you resolve the best move... that's where the value is." Tapping `best` animated the line on the board but never wrote it out, so it vanished as soon as the animation finished. It now prints the whole sequence as text under the move.
- TALL-SCREEN RESIDUE: on a tall narrow phone the board is already at its width cap and cannot take the leftover height, so the player bars open further (max 116 above 900px tall, 96 otherwise, and a 32px minimum below 640px tall where every pixel counts). Gap above the board went from 28px to 8px at 430x932.
- GATES: compile clean. Review gate at 430x932 and 390x844, 25 assertions each, GATE PASS including the new icon controls found by title, both bars reading "speedo23 WHITE" and "Kunal2023 BLACK", gaps of 4-8px, and the analysis board still accepting a real dragged move. Fit sweep 14/14. Puzzle gate and play/lesson regression re-run. Brilliant-gate guard unchanged.
- FEEDBACK SWEEP, at Kunal's request ("scroll through this chat over the last day and pick up all the feedback you haven't picked up"): 28 items reconstructed, 18 already live, 6 shipped in this build, 4 genuinely open (base shell on play/lessons/puzzles; the audit-page taps which are now largely obsolete since the black band was diagnosed analytically; the deferred multi-threaded engine; and his answer that he opens the app from the home screen icon, which is what exposed the missing bottom inset). Published to him as a page. NOTE THE LIMIT: this session lost its earlier history to a context compaction, so pre-compaction feedback was reconstructed from a summary rather than read back verbatim. If an item was dropped, that is where it went.


## 2026-09-11 - BUILD #343 - Why a move got its verdict, an analysis board, three engines at once, and the base shell
- Three of Kunal's asks in one ship. #342 was built and gated as its own change (the reasons and the analysis board) but never uploaded on its own; it is live inside #343, so there is no #342 commit.
- WHY THE MOVE GOT ITS VERDICT (his ask: "what we don't have right now is why something is a good move, why something is a brilliant move, why something is a blunder"). The one-liner under the move used to restate the label. It now composes a real reason from four things the engine already knew and we were throwing away: what the move cost, the opponent's best reply (the punishment), the move that held the position instead, and the tactical motif. To get there: sfEval1's worker parser now carries the multipv-2 first move through to the caller (first:_fm -> alt), aU[] threads the runner-up move through the engine loop AND the eval cache, each analysis row gained altSan/altDrop/motifs, and a new moveMotifs(pos,mv) detects mate, check, discovered check, fork, capture and promotion. Live example off the gate: "A blunder, about 4.1 pawns of advantage gone. Ng4 is the punishment. Nxd6 held it. White is losing here."
- ANALYSIS BOARD (his ask: "there should be an analysis button that we can hit and then we can start playing and see how something works out"). A second control row under the transport row: star Next key moment / Analyze when reviewing; Undo / Exit analysis once you are in it. Analyze hands you the board at that exact ply. Both colours are movable, the move line becomes "Analysis - N moves in - White to play", Undo steps back one, Exit returns to the reviewed move, and stepping to another ply leaves analysis on its own. The engine line follows whatever you play, so the readout is about YOUR position, not the game's.
- ONE NUMBER ON SCREEN (found while gating the above). The eval bar and the engine line came from two different searches at two different depths and openly disagreed: bar -3.2, line -5.0, same position. In review with the engine readout on, both now read the same search. sfBestLine gained an optional live-score callback so the line shows the real score instead of the static material eval it was using.
- PARALLEL REVIEW, 61s -> 27s MEASURED (his ask: "our evaluation takes much longer than chess.com's"). A review is 60-80 INDEPENDENT position evaluations, so it parallelises almost perfectly: a pool of single-threaded Stockfish workers, each with its own callback slot, pulling from one work queue so a slow position never idles the others. No SharedArrayBuffer, no cross-origin isolation, nothing new to download. Pool size adapts (min(4, hardwareConcurrency-2), capped at 2 when navigator.deviceMemory <= 4, each worker pinned to Hash 16 so four copies do not blow up an older phone), workers are terminated the moment the review ends, and localStorage ct_pool overrides it for testing. The saved time is partly spent back on quality: the per-position budget went from ~1333ms to ~2270ms (target ~25s wall clock instead of ~60s), so the review is both faster AND deeper than #341.
  - MEASURED in the gate, same game, same machine, 44 plies: 1 worker 61s; 3 workers 27s. The progress screen now says "Running 3 Stockfish engines side by side".
  - The eval cache key no longer includes movetime (it would have missed a valid cache just because the pool came up a different size); it is version-tagged sf18c instead, and the cache is now checked BEFORE any worker is created, so a re-review still costs nothing.
- MULTI-THREADED STOCKFISH: NOT SHIPPED, and the reason is worth recording. Kunal said yes to serving COOP/COEP from the service worker to unlock a threaded build. Two findings killed it for this build. (1) The threaded binary is not obtainable from this sandbox: npm, jsdelivr and cdnjs are all blocked, github.com clone works but the Stockfish WASM repos hold C++ source with no prebuilt artefacts, and emscripten cannot run here. (2) COEP require-corp would break the app: Firebase/gstatic, the chess.com API and Lichess are all cross-origin and do not send CORP, so isolating the origin would cut them off. COEP credentialless avoids that but its Safari support is the open question. Shipping the two headers with no threaded engine behind them would have been pure risk for zero gain. Parallel workers were built instead, and for THIS workload (many independent shallow searches, not one deep one) they are the better tool anyway.
- BASE SHELL, first screen (his proposal, agreed): back arrow and dots, one context line, board at full width, one row of controls, and the player bars filling the slack. Built on the review screen. This also closes the black band he flagged twice: a square board on a 932px phone leaves ~190px over, the layout was splitting it into dead space above and below, and the bars now absorb it (flex 1 1 0, min 46, max 96). Bars are named from the PGN headers, show captured material and the material lead, and ring whichever side is to move at this ply. Captured material now reads from boardGame rather than game, so it follows the review and the analysis board instead of the played game.
  - MEASURED at 430x932: board 424px unchanged, gap above the top bar 4px, gap below the controls 8px, bars 95px each, no page scroll. At 390x844: board 384px, gaps 4px and 8px, bars 71px each, no page scroll.
- GATES: esbuild 0.27.7 compile CLEAN (React 18.3.1 reconstituted from the live bundle, npm still blocked). Review gate in real headless Chromium at 430x932 AND 390x844, 25 assertions each, GATE PASS: mount, gallery card, 3-worker parallel review, wall clock under a minute, reason line explains the verdict and names a punishment move, board full width, no page scroll, analysis row renders, both player bars present and named from the game, no dead band above or below, Analyze enters a board that accepts a real dragged move, Undo steps back, engine line and eval bar agree to the digit, Exit returns to the reviewed move with the reason back, zero page errors and zero app console errors.
  - BRILLIANT-GATE REGRESSION GUARD: PASS by byte-comparison against live #341. classify, seeSq, brilliantGate, isBrilliant and isAttacked are all byte-identical (sha256 prefixes 3fd9a1d5, 79c22471, c11fb219, 96a65db0, a7e25342), and the thresholds 220 / 1.2 / 4.5 / -1.0 / 90 each still occur exactly once in brilliantGate. Bxh3 (loss 101, sac 2, evAfter 2.86, evBefore 3.87, cap 220) passes and Bc7/Bxf6 fail by construction, since not one byte of the deciding code changed.
  - audit.py: NOT re-runnable this session (pypi still blocked, python-chess absent). lessons.js is byte-identical to the file the last audited commit checked (git diff empty), so the 170 PASS result is unchanged by construction. Documented deviation, not a skipped gate.
- SHIPPED AND VERIFIED ON THE WIRE: commit 305dbde7cb441ab2ceab867fa1976b2e5a621c49 on main, uploaded through GitHub's web UI in Kunal's Chrome (git push is still refused: "LearnToCheckmate/chess-trainer is not in this session's authorized repository set" - the fix for next time is to add the repo as a session source). app.js fetched back from raw.githubusercontent.com at main is BYTE-IDENTICAL to the gated build, 896,275 bytes, stamp "#343 2026-09-12 analysis board, coach reasons, parallel review".
- GALLERY HOUSEKEEPING: 15 older cards were still wearing "(NEW)". Only the newest card carries it now, and the #343 card is first in the list.
- GATE BUGS FOUND AND FIXED, worth remembering: (1) the review board rejected moves in the harness because the script pressed the SOURCE square twice - the second press deselects, so it must tap the piece then tap the target; (2) the target-dot detector counted the round verdict badge as a dot on every square, so it always "found" a target; the fix is to diff the dot set before and after selecting.


## 2026-09-11 - BUILD #341 - Review screen, the chess.com pass, plus a Stockfish cache
- Kunal's six points on the review screen, all built: (1) the tab bar is gone from the review move screen, which hands its 62 px to the layout; (2) the way out is a single back arrow top left, with the three dots as the only other control; (3) the reason a move got its verdict is back as ONE line of plain text under the move line, no box (he asked for the box gone in #337 and for the reason back here, so it is now text, not a panel); (4) thin arrows flank the move strip for one-tap stepping; (5) "Analyze with the engine" in the three-dots sheet shows the engine evaluation and the line it wants, in real notation, following the ply he is on; (6) the best-move chip plays the line out on the board as before and now also prints the sequence.
- STOCKFISH CACHE (he asked: "once I do an eval, can we save it down"): the engine pass now writes its own output (white-POV centipawns per ply, the second line, the best move per ply) to localStorage keyed by the exact move list plus the movetime budget; everything the review shows is derived from those in pure JS. Re-opening a reviewed game skips the engine entirely. MEASURED in Chromium: first run 62 s, same game again 4 s. 24 games kept, least-recently-used dropped.
- WHY OURS IS SLOWER THAN CHESS.COM (he asked): not depth, structure. Chess.com evaluates server-side on real hardware and caches per position across all users, so a review is a lookup. We run Stockfish 18 lite WASM single-threaded inside his phone browser, one position at a time, with a fixed ~60 s budget spread over the game (movetime = 60000/(N+1), clamped 700-2000 ms). Options, none built yet: run 2-3 engine workers in parallel (roughly halves it, costs ~7 MB each); serve COOP/COEP headers from the service worker to unlock SharedArrayBuffer and a multi-threaded build (3-6x, needs a new wasm binary in the repo); skip book plies and spend the budget on the middlegame.
- SCREEN AUDIT (measured at 430x932, his phone): review move screen board 424 px of 430 (the maximum, since the root keeps 3 px each side); puzzle browse and the brilliancy drill 416 (they still add the legacy 4 px reserve); play vs computer 416, with a 44 px opponent bar, the eval strip, a 44 px player bar, then a 111 px row that carries "vs Computer, Elo -/+, slider" plus the six control buttons; lesson watch and practice 416, with a 61 px coaching line, a 205 px text block above the board and a 99 px MOVES panel below. Gallery card "Review screen, chess.com pass (NEW)".
- STILL OPEN for the base-layout decision: whether play, lessons and puzzles adopt the same shell (back arrow + dots, board at full width, one line of context above, one row of controls below, no tab bar), and what fills the vertical slack a square board always leaves on a tall phone. Chess.com fills it with player bars; that is the obvious candidate.
...
- ROOT CAUSE, found by dumping every root child rect in Chromium: the app renders an aria-hidden SPACER, height calc(62px + safe-area-inset-bottom), whose only job is to reserve room under the fixed tab bar. It carried NO css order. The puzzle screen is the one screen that orders its children (text order 1, board order 2, controls order 3), and an unordered flex child defaults to order 0, so on that screen alone the spacer jumped to the TOP as an empty band AND stopped reserving anything at the bottom. Measured before: empty 62 px block at y 8, board 543..959 against a tab bar at 876 (83 px of board hidden), the Hint/Show move/Reset/Review/Next row entirely below the fold at 968..1073, page scrolling 1081 in a 932 viewport. On his phone the band is 62 + his home-indicator inset, which is why it looked like a third of the screen. FIX: order 99 on the spacer, so it is always last. That one line fixes both symptoms.
- Also cut, because the same screenshot showed them: (1) replaying your own brilliancies or mistakes (mistakeMode) no longer renders the Lichess daily panel or the dev loaders, which belong to the Lichess view, not to your own positions; the head row now reads "‹ Review" plus "✨ Your brilliant moves · 2 of 5" instead of "‹ Roadmap / Lichess · N solved" (216 px saved). (2) The PIN-unlocked dev loaders (puzzle ID, pack URL) are collapsed behind a small "dev loaders" link even in the Lichess view. (3) The message box no longer reserves a fixed 74 px when there is no message. (4) Leaving a drill by the tab bar or by choosing Online puzzles now clears mistakeMode, which previously left the Lichess view without its own panel.
- BOARD SIZING, now measured rather than guessed: the puzzle text block and the control block are measured with a ResizeObserver into pzStackH, and the board height cap is viewport - safe top - pzStackH - tab bar - gaps (floor 232). No feedback loop: both blocks are now sized to min(vw-8,440) instead of the board width, so their height cannot depend on the board. Two flexible spacers (order 0 and 98) centre the stack, since a square board on a tall phone always leaves slack.
- Measured after: 430x932 board 256..672 fully visible, controls end 784 above the tab bar at 876, no page scroll; 390x844 board 231..607, controls end 719, no scroll. Known limit: 320x568 (iPhone SE) still scrolls about 90 px because the control row wraps to two rows there and the board floor is 232.
- GATES: new pzgate drive at 430x932 and 390x844 (drill, Lichess view, free-play browse): board fully visible, controls above the tab bar, no page scroll, no empty band, panel hidden in a drill, dev loaders collapsed then opening on tap, zero errors. Review-screen regression gate re-run and still PASS at 390 and 320 (the spacer and SQ changes touch every screen). Gallery card "Puzzle screen: the board fits again (NEW)" opens Free play browse.
...
- BUG FOUND AND FIXED (not taste): the review board was sized to the full screen width and the eval bar (22 px in review, 14 px vs the computer) was added beside it, so the row was wider than the phone and the root's overflow:hidden clipped BOTH sides: the eval number on the left, the h-file on the right. Reproduced exactly in Chromium at 390x844 with a real analyzed game (board right edge 398 on a 390 screen). Fix: the SQ formula now reserves the bar width plus the root side padding. Board is 352 px on a 390 phone with the bar, 384 without. The same bug existed in games vs the computer (8 px clipped); fixed by the same line. The Sep 8 UI sweep missed it because it looked at the Review screen without a loaded game.
- EVAL CHIP (point 4, shipped in both layouts): the move line now carries a chip next to the verdict: light chip when White is better, dark when Black is, plus or minus in pawns from White's side, +M5 / -M3 for a forced mate. Same number the bar shows, readable.
- ONE-SCREEN LAYOUT (points 1 and 2), shipped as a PREVIEW behind the first gallery card, off by default, because which elements go is his taste call: compact head row (Summary, players, three-dots), board sized to leave room for the rows below, one move line (move, verdict, eval, ply counter), two-line plain explanation with the Better-was line, one control row (first, prev, play, next, last, key-moment star), the move strip. Everything else moved into a three-dots sheet: counts (tappable), eval graph, best move, play from here, flip, copy PGN, eval bar toggle, gate numbers, Classic review layout, new game. Measured in Chromium: 390x844 fits with no scroll (page height 844); 320x568 scrolls 53 px (board floor 296). The decision page for it: artifact "Review Screen Audit" 9597e210-cb6c-4174-99d6-204d256dfb56, collection "decisions", ids R1..R6 and Q1; read it next session and build only what he approved.
- BLACK BAND (point 3): NOT reproduced. In Chromium the tab bar is flush at the bottom at every size. Hypotheses: the app opened in a browser tab whose bottom toolbar collapsed (iOS Safari or Chrome), or the WebKit keyboard-dismiss offset on a fixed element. Second gallery card "Layout numbers" toasts window height, screen height, visual viewport, dvh, the bottom safe-area inset and whether it runs from the home-screen icon; Q1 on the audit page asks how he opens the app. Fix follows the numbers.
- GATES (real Chromium, 390x844 and 320x568): mount OK, zero errors; classic layout board inside the viewport with the bar visible; eval chip text; no horizontal overflow; diag toast; compact layout renders, fits at 390, sheet lists every item, Classic review layout switches back. Gallery cards: "One-screen review, PREVIEW", "Board fits the screen again, eval on every move", "Layout numbers for the black band". audit.py not re-runnable here (pypi blocked); lessons.js untouched since #330.
- GOTCHA for cards that import a game while a review is open: clear the old review first (setReview(null)) or the analyzing panel renders above the old board and any measurement taken then is wrong; cost one gate run.

## 2026-09-10 - BUILD #332 - Gallery card fix, found by verifying #331 LIVE in Kunal's Chrome
- #331 went live as commit 5f745f8 (GitHub web upload through Kunal's Chrome; all six blob shas matched the local signed commit; raw at the hash and the Pages host both serve the #331 stamp). Driving the new card on the LIVE site showed the staged rows being replaced within seconds by his real 20-game list: the Review screen's one-time auto-fetch fires on the first entry into analyze mode and mergeGames overwrote ccGames. The sandbox never showed this because Chess.com is blocked there, so the fetch failed and the staged rows survived. Card fix: set gamesAutoRef before entering analyze mode and seed ccRawRef with the staged games. Real EST rows did appear on his real games in the meantime (2 within seconds), which is the feature working as designed.
- No product change beyond the card. Compile clean; mount check and card drive in headless Chromium as for #331.

## 2026-09-10 - BUILD #331 - Brilliant pills: estimates vs reviewed, no more silent downgrade (readout-driven)
- THE READOUT (step 1). All four Sep 10 games pulled from api.chess.com via Kunal's Chrome (the sandbox blocks that host), run through the app's OWN gate code (brilliantGate, seeSq, loadSANs bundled from chess.jsx) in headless Chromium with the repo's Stockfish 18 lite at fixed depth 18, MultiPV 2, white-POV cp, loss = eval drop from the mover's side, loss forced to 0 when the played move is the engine's bestmove (same rule as the review path). One correction to the brief first: the "bishop takes g5, +3.78 after" brilliancy is NOT in the dev_mooie game. It is 14. Bxg5 in the game vs gomdz (Kunal White, won by resignation, chess.com/game/live/174294340246). The dev_mooie game (Kunal Black, won on time, 174295698562) contains no Bxg5 and no move that our gate or a plausible reading of Chess.com's would call brilliant; its only sacrifice-shaped moves (19...Qxf6, 20...Re1, 22...Qxf1+) all start from +6 or better, above the 4.5 evBefore ceiling, so "Brilliant 0" there is the gate working as designed. I could not read Chess.com's own review of that game (needs his logged-in premium review page), so their count for dev_mooie is unverified.
  - gomdz, 14. Bxg5 (ply 27): DEEP loss 0 (it IS the engine's best move), sac 2 (bishop for pawn, SEE-confirmed), evBefore 3.90, evAfter 3.91, cap 220 (sac>=2 and evAfter>=1.2) -> PASS, Brilliant. Chess.com's +3.78 matches. SHALLOW (the list pass, rankMoves depth 2 + static eval): loss 130, sac 2, evBefore -0.25, evAfter 0.70, cap 90 -> FAIL on both loss and evAfter. So the gomdz brilliancy can only ever appear after a full review; the near-best cap is NOT the blocker, the shallow eyesight is.
  - vinnimt, 19...Rd4 (ply 38): DEEP loss 0, sac 5, evBefore -1.53, evAfter -1.51 -> FAIL (Black is worse; a rook sac in a lost position is not a brilliancy). SHALLOW: loss 0, sac 5, evBefore +1.15, evAfter +1.15 -> PASS. Phantom estimate pill; the review correctly removed it; the old UI erased it without a word.
  - mr_dhanzzxnsx, 12...Bxd4 (ply 24): DEEP loss 384, sac 2, evBefore 2.28, evAfter -1.56 -> Blunder. SHALLOW: loss 215, sac 2, evBefore 2.80, evAfter 4.00, cap 220 -> PASS. Same phantom pattern (the relaxed 220 cap plus a static eval that cannot see the refutation).
  - Regression guard, same harness: Harris 17...Bxh3: DEEP loss 92, sac 2, evBefore 4.52, evAfter 3.60, cap 220 -> fails at depth 18 in the sandbox ONLY on evBefore 4.52 vs the 4.5 ceiling; Kunal's phone recording (loss 101, sac 2, evAfter 2.86, evBefore 3.87) passes, and the gate is unchanged in this build, so his confirmed result stands. FLAG for Kunal: the 4.5 evBefore ceiling is knife-edge for Bxh3 across engine depths; if he ever sees it drop to ?! on device, the fix is a ceiling of 5.0, not a cap change. Harris 11. Bxf6: sac 0 (trade) -> FAIL, correct. papakehtehain69 13...Bc7: sac 0 (retreat) -> FAIL, correct. No false positives anywhere in the deep pass across all six games (gomdz, vinnimt, mr_dhanzzxnsx, dev_mooie, harris, bc7); every deep verdict is listed in readout.18.json in the build dir. Local commit for this build is signed (SSH) and authored Claude <noreply@anthropic.com>; the hash is in the push report.
  - The missing 5th argument (step 2): analyzeGameCounts called isBrilliant without evalBefore. brilliantGate's null check fell back to evalPawns(pos), which is exactly the value the fix now passes explicitly, so this was a latent bug, not a verdict-changing one: across all 286 plies of the six games, shallow-with-fix and shallow-without-fix agree on every verdict (0 diffs). Fixed anyway in both places (background pass and the no-Stockfish review fallback) so the gate is never fed undefined.
- WHAT SHIPPED (steps 2 and 3). (a) evalBefore passed explicitly in analyzeGameCounts and in the fallback review path. (b) Every gameStats entry now carries src:'est' (background pass) or src:'review' (full review, plus engine:'sf'|'fallback'). A legacy entry with no src is treated as an estimate. (c) List row: an estimate row is dimmed to 78 percent, carries a small EST tag, and its brilliant pill is hollow (dashed cyan border, no fill, no glow) and reads "!! 1 brilliant?". A reviewed row shows the solid glowing pill as before. (d) Never silently downgrade: when a full review finds fewer brilliants than the stored count, the new entry keeps was:<old count> and the row shows one line "review found 0 brilliant (estimate said 1)". A later review that finds the brilliant again clears the note; a re-review with the same lower count keeps it.
- CALIBRATION (step 4): NO gate change, by evidence. Bxg5 passes the deep gate with loss 0, so raising the near-best cap would not have changed anything for it and would only widen the phantom vector the mr_dhanzzxnsx case shows (loss 215 already slips under 220 in the shallow pass). Shallow and deep now "agree" on vinnimt and mr_dhanzzxnsx in the only honest sense available: the list shows why they differ (EST pill before review, downgrade note after).
- KUNAL DECISION (not taken unattended): on this sample the estimate pass was 0 for 4 on brilliants (three phantoms, one miss). Option A: keep the hollow "brilliant?" pill (shipped). Option B: drop brilliant from the estimate pass entirely and let only reviews award it. B is one line; say the word.
- GATES: compile clean (esbuild 0.27.7 with React 18.3.1 reconstituted from the live #330 bundle, since npm is blocked this session; bundle delta vs #330 is the code change only). Mount check in REAL headless Chromium at 390x844 (jsdom is not installable this session; Chromium is the stricter check): root renders, body contains CHESS TRAINER, zero page errors, zero app console errors, before and after driving the card. Gallery card "Brilliant pills: estimates vs reviewed (NEW)" driven: gstat-est 1, bril-pill-est 1, gstat-rev 2, bril-pill 1, bril-downgrade 1, row text asserted. audit.py: NOT re-runnable this session (pypi blocked, python-chess absent); lessons.js is byte-identical to the file #330 audited (git diff empty), so the 170 PASS result is unchanged by construction. This is a documented deviation, not a skipped gate.
- SANDBOX NOTES for the next session: raw.githubusercontent.com, api.chess.com, api.github.com, npm and pypi were all blocked; git clone over github.com works but PUSH does not unless the repo is added to the session's sources when the task is started (see HANDOFF 5a; #331 went out through a GitHub web upload of the six files instead); Kunal's Chrome (Browser 1) fetches api.chess.com fine; Playwright Chromium at /opt/pw-browsers/chromium runs the repo's Stockfish worker at real depth (readout.js in the build dir, about 100 s per game at depth 18).

## 2026-09-08 - BUILD #330 - First endgame videos + two stale entries closed by evidence
- ENDGAME VIDEOS (the ENDGAMES category had none): Bishop & Knight Mate -> oX_pq-2kiJI "Knight and Bishop Checkmate | Chess Endgames" (30 min); Queen vs Pawn (on the 7th) -> Ji8NjJknEjY "Queen vs Pawn Endgames" (17 min). Both confirmed on their watch pages. 82 -> 84 of 170.
- HELD ON PURPOSE, do not re-search: no HP video exists specifically for Lucena, the opposition, rook vs pawn, or the two-rook mate. Compilations that TOUCH our lessons but are not about them: "8 Most Important Rook Endgames" f2ASOe8za3w (Lucena/Philidor), "5 Most Beautiful Checkmate Patterns" KKhsd1JdIYs (Smothered, Anastasia's), "10 Most Important Chess Endgames" 4yxObBZa3dY, "King and Pawn vs King and Pawn" AyK_8WTL7C8 (our lesson is K+P vs K, a different position). A compilation that merely mentions a lesson is not a walkthrough of it, so those lessons stay video-less. Also note: HP's "Philidor" videos are the Philidor DEFENSE (an opening), not the Philidor POSITION (our endgame draw) - never wire those together.
- CLOSED BY EVIDENCE, no code change: the old known issue "clicking a variation should reset at the branch hub, not replay the trunk" is ALREADY FIXED. Driven in jsdom on the live bundle: on Italian Game, picking Evans Gambit (trunk and variation share 6 plies) lands the demo on 3...Bc5, the branch point, not move 1. pickVariation computes the common prefix and sets demoPly to it. Struck from the issue list.
- UI SWEEP, no defects found: every main screen (Home, Puzzles, Discover, Openings list, lesson focus, lesson sheet, New Game, Review, Menu) rendered in real Chromium at 320x568, 390x844 and 430x932. Page horizontal overflow 0 px everywhere, zero elements off-screen, zero clipped text nodes, zero app console errors. The iPhone SE width was the one I most expected to break; it does not.
- TRACKER TRUTH-UP: "Lesson videos (embedded player + curated IDs)" was a duplicate of the walkthrough row, marked done and deduped. "Review overhaul" note corrected: the brilliant heuristic was closed back in #307 by Kunal's own Bxh3 recording; only the eval-bar keep/remove decision remains, and that one is his.
- Gallery card "First endgame videos (NEW)". Verified in jsdom: card lands on Bishop & Knight Mate, title + author render, 84 videos counted; compile clean; audit PASS 170; mount check both widths.
...
- LIMITS, stated plainly: STUN only (no TURN), so symmetric-NAT networks (some offices, some mobile carriers) will fail with the message above; Kunal's backlog already lists a TURN service as his infra item. Not verifiable end to end from here: needs two signed-in devices in one online game. What IS verified: full jsdom drive with stubbed media/RTC through ring -> Answer -> live tile -> Hang up, with the exact gamePush payloads asserted (answer, hangup), 0 runtime errors; compile clean; audit PASS 170; mount check both widths.
- Gallery card "Video call in online play (NEW)": seeds a demo online game (opponent "Coach" already calling) so the ring bar is visible on any device without a second player; Answer asks for camera permission and then waits (demo opponent is not real).
- FLUSHED: "21 new opening videos" (verified live in Kunal's Chrome on #324: card lands on Berlin, Watch in app loads youtube-nocookie embed PB9H-qtSsUM and the player shows "Berlin Defense · Complete Chess Openings, Hanging Pawns"). Also confirmed there: #323's sw fix works, a plain navigation showed #324 within a minute of the push.
- Gallery now 3 cards: Video call (NEW), New Game on one screen, Focus mode stage C.
- Harness: mountcheck.js now stubs HTMLMediaElement.play/pause (jsdom "not implemented" noise, not an app error).


## 2026-09-07 - BUILD #324 - 21 opening videos, channel-confirmed via Kunal's Chrome (new sourcing path)
- NEW SOURCING PATH, strongest yet: Claude drove Kunal's Chrome to youtube.com/@HangingPawns/search?query=<opening> (channel-scoped, so every hit is Hanging Pawns by construction), then opened each watch page and read ytInitialPlayerResponse.videoDetails for id, title, author and length. No search-engine guessing, no memory. YouTube fetches from the sandbox are 429-blocked; Chrome is the way.
- ADDED (lessons.js only, no line changes, audit PASS 170): Colle FN7xJhjCxkY; Bishop's Opening wgeshBzJNh0; King's Gambit Bishop's Gambit eN-NroFECXo; Vienna Gambit NKeSyWouCPc; Petroff KrGPeutGfaQ; Berlin PB9H-qtSsUM; French Advance plgs6rbCMjE; French Exchange YlA3heOMMN0; French Tarrasch IGS68T5FmvM; Tarrasch Defense esjT83wWr78; Caro-Kann Advance m9uHKJ1WgbM; Caro-Kann Classical OiMf511N_xc; Caro-Kann Panov RUBEZ95_BiQ; Nimzo-Larsen lnMg7otrVpE; Bird wtTk2pd1xlk; Taimanov urpEN-6XDoY; Kan 9CxgfDX44Ug; Pirc Austrian 7W2dB74VcUQ; Leningrad Dutch FE1dYi-c1kU; Chigorin CASr3pqMjYk; Modern Benoni x0TDJK973ms (this last one is HP's general Benoni intro, the closest they have). Videos: 50 -> 71 of 170 lessons.
- SEARCHED, NO HANGING PAWNS VIDEO EXISTS (checked on the channel, do not re-search): Four Knights (still only a 2018 game recap), Benko, Winawer, Budapest, Latvian, King's Indian Attack, Sveshnikov, Scheveningen, Alekhine, Schliemann, Marshall Attack, Rossolimo, Ragozin, Torre. Vienna Game (non-gambit) HP video FV67KSxkJQw exists if a Vienna Game lesson is ever added; Ruy Lopez Chigorin (Closed) E8ek3cfswcQ could serve "Ruy Lopez: Closed" (not applied: it is the Na5 sub-line, not the closed system overview).
- Gallery card "21 new opening videos (NEW)": opens Berlin Defense with the video box expanded in the 3-dot sheet. Verified in jsdom: card lands, title + author render, 71 videos counted from CTLESSONS, 0 errors.
- Also this build: tracker dedupe (the review "Best-move play-out" row was a duplicate of the polish row; marked done). Kunal's roadmap feedback (read from the artifact db): "buy the domain next, then build everything that does not need me one by one without stopping" - acting on it.
- DOMAIN: gambitcoach.com is TAKEN. Checked live at Cloudflare in his Chrome. Available .com at $10.46: gambitgym, gambitpath, gambitschool, opentheboard, castleandmate, squareonechess, learntocheckmate. Taken: gambitlab, gambitgo, gambitclub, gambiteer, dailygambit, yourgambit, playgambit, movebymove, brilliantmove, playbrilliant, checkmatecoach, checkmatelab, openingcoach, knightcoach, knightschool, forkandpin, pawnstorm, pawnup, boardwise. Purchase is Kunal's click (blocked for Claude by design); DNS + CNAME file are Claude's once bought.
...
3. DAILY 3: Home card (both phone and iPad-landscape layouts) = 1 lesson + 2 puzzles per day with checkmarks and a done state; hooks ride the existing bumpDaily call sites (2 puzzle solves, finishRep for lessons); completion toast; state ct_daily3 (synced).
6. BRILLIANT VISIBILITY (Kunal screenshots 2026-06-29, Englund Rosen Trap win vs Shironesh): (a) games-list rows now show a glowing cyan "!! N brilliant" pill instead of easy-to-miss tiny glyphs; (b) BUG FIX exposed by his screenshots: the Summary table counted a brilliant played within book plies as Book (showed Brilliant 0 while the move view said !! and the list said !! 1) - Brilliant/Great labels now take precedence over the book bucket in _sideStats, which also fixes the "Clean game" headline to celebrate the brilliancy. Gallery card "Brilliant badge on games list" seeds a demo row (no network needed) to show the pill; verified in jsdom (pill text + gradient style render, 0 errors). His real fetched games get the pill from the already-persisted ct_gamestats tallies.
5. BRANCHES FROM THIS OPENING (Kunal request 2026-06-29, voice note: which gambits come out of the Italian or the Four Knights): every lesson now shows a Branches row of all top-level lessons sharing its first 5 plies, computed from the move lines (no manual tagging), gambits/traps/attacks highlighted gold first, chips tap-to-open. Renders on BOTH the phone path (!railed) and the iPad rail path. Gallery card "Find gambits by opening" opens the Italian Game lesson. Verified in jsdom on both widths: row renders, Evans/Fried Liver/Nakhmanson chips present, chip navigation works, 0 runtime errors. For the record, the Italian family in-app: Evans, Fried Liver, Nakhmanson, Jerome, Blackburne Shilling, Rousseau, Legal Trap, Traxler, Max Lange; Four Knights family: Halloween, Belgrade.
4. COACH'S TAKE: in the Review Summary, under the opening name: plain-English engine-derived read (which phase had the trouble, toughest single moment with pawn loss, blunder-habit nudge) + "Train this opening" (LIB fuzzy match on detected opening) + "Sharpen with puzzles" buttons. Pure client, no new backend.
- DEFERRED with reason: the lesson-data bundle split (Kunal also green-lit it). It is the highest-regression-risk item (makes LIB async through a 210-useState component) and does not belong in the same uncommitted mega-batch as four features; it gets its own dedicated build with device verification. Queued under CLAUDE can build now.
- Gallery: NEW card "Daily 3 + toasts" (jumps Home, fires a sample toast); PGN-import card note extended to point at the Coach's take box. Live cards now 5 - above the lean 2-3 target because THREE builds are stacked unverified awaiting the token; will flush hard once Kunal records.
...
- 7 NEW gambit lessons appended to MORE (concat-last, LIB indices stable). Library 163 -> 170 lessons. Chosen by Kunal from the tap-to-pick, sourced from Chess Vibes "7 Most Successful Gambits" (all of that video's actual picks were ALREADY in the app, so these are genuinely-new additions instead):
    ...
    3. Marshall Gambit (Semi-Slav, W) - respected, genuinely sharp.
    4. Nakhmanson Gambit (Italian, W) - OBJECTIVELY UNSOUND piece sac, framed as such; viral surprise weapon.
    5. Fajarowicz Gambit (Budapest, B) - offbeat, White keeps an edge, tricky.
    6. Frankenstein-Dracula Gambit (Vienna, B) - legendary tactical mess, roughly balanced.
    7. Wing Gambit (French, W) - dubious but unbalancing, off-theory.
- Each lesson: honest idea + per-move notes (no move-label repetition, no double-quotes), framed accurately (which are sound vs dubious vs unsound). No name collisions with the existing 45 gambit lessons.
- Fried Liver Attack lesson video (from earlier this session): Hanging Pawns VxTPwHQ1Rv8, web-sourced via direct YouTube URL + lichess + third-index corroboration. Liveness not device-confirmed (no YouTube from sandbox).
- Gallery cards this build: "7 new gambits (NEW)" (opens Milner-Barry lesson) + "Opening videos (NEW)" (opens Fried Liver + video box). Lean at 3.
- VERIFIED in-sandbox: esbuild compile CLEAN; audit.py PASS (170 lessons: legal lines/FENs, aligned notes, correct #/+); Stockfish 16 sweep over all 170 = only Nakhmanson flags among the new ones (304cp, expected + correctly framed as unsound), zero silent mislabels; jsdom drive of the live #309 bundle: both new gallery cards land on their lessons, idea text + video box render, 0 runtime errors.
- Added a real Hanging Pawns walkthrough video to the Fried Liver Attack top-level lesson (was video-less). ID VxTPwHQ1Rv8, title "Fried Liver Attack | Italian Game Theory", author Hanging Pawns. SOURCED via web search (not memory): direct YouTube watch URL carrying the Hanging Pawns Patreon description, corroborated by lichess and a third index, exact title match. Could not device-confirm liveness (no YouTube reachable from sandbox); on-device verification is the gallery card below.
- HONESTY NOTE on this batch: only Fried Liver met the direct-YouTube-URL confirmation bar this pass. Evaluated but HELD: Four Knights (Hanging Pawns video not published until 7/1/2026 per their own schedule), Colle System (no clean ID), Tarrasch Defense (only a lichess path-id, which the Fried Liver case proved is not always the channel's real YouTube id - medium confidence, not embedded). About 116 top-level lessons still video-less.
- Repointed the "Opening videos" gallery card to open the Fried Liver Attack lesson and auto-expand the Watch-it-explained box.
- VERIFIED in-sandbox: esbuild compile CLEAN (no error/warning); audit.py PASS (163 lessons); full Stockfish 16 sweep over all 163 lessons = 12 expected named-trap/gambit flags, all correctly framed, ZERO silent mislabels; jsdom drive of the live #309 bundle: gallery card lands on Fried Liver Attack, video title + Hanging Pawns author render, 0 runtime errors.
