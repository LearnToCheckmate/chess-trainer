...
const states={
  ...
  'why-open':async(b)=>{await exitAnalysis(b);await movesAt(b,0);let found=false;for(let i=0;i<14;i++){await b.page.locator('button[title^="Next key moment"]').last().click();await b.settle(450);if(await vis(b,'[data-ct="rev-playout"]')){found=true;break;}}
    ...
    await b.tapCt('rev-playout',300);await b.page.locator('[data-ct="rev-engline"]').last().waitFor({state:'visible',timeout:12000}).catch(()=>{console.log('     NOTE: rev-engline did not appear within 12 s of rev-playout. This state CONTINUES without it, so any assertion about the engine line below is not measuring what it says. Raised from 6 s at #382: the test lane measured this line taking up to 8 s on a real device, and the old timeout SWALLOWED its own expiry, which is the worst of both - too short AND silent. In this container it appears in 8 ms.');});await b.settle(2500);},
