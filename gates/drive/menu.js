...
async function fromTile(b,tile){await closeOverlays(b);await b.home();await b.tile(tile);await b.settle(500);await tapHeaderMenu(b);}
...
// any zIndex-1100 sheet (feedback, account, look) covers the Home tiles and hides the gallery button: close it first
...
async function readoutText(b){return b.page.evaluate((F)=>{const o=eval(F);if(!o)return null;const cands=[...o.s.querySelectorAll('div')].filter(d=>/what THIS device computes/.test(d.innerText||'')&&d.children.length>3);cands.sort((a,c)=>a.getBoundingClientRect().height-c.getBoundingClientRect().height);const box=cands[0];if(!box)return null;const r=box.getBoundingClientRect();return {text:(box.innerText||'').trim(),lines:(box.innerText||'').trim().split('\n').map(x=>x.trim()),y:Math.round(r.top),h:Math.round(r.height),w:Math.round(r.width),fs:getComputedStyle(box).fontSize,sw:box.scrollWidth,cw:box.clientWidth};},FIND);}
// the smallest element inside the sheet whose text matches (labels with child spans are not leaves)
async function sheetText(b,re){return b.page.evaluate(([F,src])=>{const o=eval(F);if(!o)return null;const re=new RegExp(src);let best=null,bh=1e9;for(const d of o.s.querySelectorAll('div,span')){const t=(d.innerText||'').replace(/\s+/g,' ').trim();if(!re.test(t))continue;const h=d.getBoundingClientRect().height;if(h<bh){bh=h;best=t;}}return best;},[FIND,re.source]);}
// the share / copy toast (fixed, zIndex 9998)
async function toast(b){return b.page.evaluate(()=>{const t=[...document.querySelectorAll('div')].find(d=>getComputedStyle(d).position==='fixed'&&getComputedStyle(d).zIndex==='9998'&&(d.innerText||'').trim());if(!t)return null;const r=t.getBoundingClientRect();return {text:(t.innerText||'').trim().slice(0,120),y:Math.round(r.top),h:Math.round(r.height),bottom:Math.round(r.bottom)};});}
...
S['menu-readout']=async(b)=>{await S['menu-discover'](b);await setToggle(b,/^Layout readout/,/shown$/);await b.page.evaluate((F)=>{const o=eval(F);const cands=[...o.s.querySelectorAll('div')].filter(d=>/what THIS device computes/.test(d.innerText||'')&&d.children.length>3);cands.sort((a,c)=>a.getBoundingClientRect().height-c.getBoundingClientRect().height);if(cands[0])cands[0].scrollIntoView({block:'center'});},FIND);await b.settle(200);};
...
module.exports={states:S,sheetText,toast,setToggle,closeOverlays,sheet,hasSheet,rows,leaves,scrollSheet,tapRow,closeSheet,tapHeaderMenu,menuBtnRects,fromTile,overlay1100,overlayText,readoutText,setStore,FIND,
